                ...
                <div class="shadow sm:overflow-hidden sm:rounded-md">
                    <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                        ...
                        <div class="md:flex md:items-center mb-6">
                            ...
                            <div class="md:w-2/3">
                                <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500"
                                       id="passwordRepeat" name="passwordRepeat" type="password"
                                       placeholder="******************">
                            </div>
                        </div>
...
<!--                                    雖然使用者無法看到＆編輯自己的管理權限，但實際上送出表單時仍有送出管理權限的選項-->
                        ...
                        <div class="bg-gray-50 px-4 py-3 text-right sm:px-6">
                            <button type="delete" name="delete"
                                    class="inline-flex justify-center rounded-md border border-transparent bg-red-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                    action="/deleteMember?<?= $user["memberID"]; ?>">
                                危險：刪除會員帳號
                            </button>

                            <button type="update" name="update"
                                    class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                    action="/updateMember?<?= $user["memberID"]; ?>">
                                修改會員資料
                            </button>
                        </div>
                    </div>
        ...
        <?php if (isset($_POST["update"])) { ?>
            <!--            <a>--><?php //= ($updateStatus === "YES") ? "修改成功！" : "修改失敗"; ?><!--<br></a>-->
            <a><?= (empty($errMsg)) ? "修改成功！" : "修改失敗"; ?><br></a>
        <?php } ?>
        <?php if ($_SESSION["ADMIN"] === 1) { ?>
